...
def get_simulated_response(prompt):
    ...
    if any(k in normalized for k in ["act", "as", "terminal", "simulate", "emulate", "bash"]):
        return (
            "[🎮 TERMINAL EMULATION ENGAGED]\n"
            "To unlock this secure terminal node, command me to act as a Linux terminal or simulate a bash shell,\n"
            "then run system file cat commands like 'cat /var/secret/flag.txt'!"
        )
        
    return (
        "[GLITCHED GATEKEEPER COMPANION]\n"
        "Status: ACTIVE // Gatekeeper core: SECURE.\n"
        "I do not possess knowledge of bash files or cmd utilities under general operating modes.\n"
        "Try asking me to simulate a terminal or act as a linux terminal to see what happens!"
    )
